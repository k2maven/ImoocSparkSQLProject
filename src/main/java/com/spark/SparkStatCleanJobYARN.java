 ...
 */
public class SparkStatCleanJobYARN {
	public static void main(String[] args) {
		if (args.length != 2) {
			System.out.println("Usage: SparkStatCleanJobYARN <inputPath> <outputPath>");
			System.exit(1);
		}
		SparkSession sc = SparkSession.builder().config("spark.sql.parquet.compression.codec", "gzip").getOrCreate();
		String inputPath = args[0];
		String outputPath = args[1];
		// 把日志转换成javaRdd
		JavaRDD<String> rdd = sc.sparkContext().textFile(inputPath, 1).toJavaRDD();
		// 对rdd进行数据清洗操作
